...
export default function AudioProcessingPage() {
  ...
  const loadRecordings = async () => {
    try {
      ...
      setTotalRecordings(allRecordings.length);
      
      // Client-side pagination
      const startIndex = (currentPage - 1) * itemsPerPage;
      const endIndex = startIndex + itemsPerPage;
      setRecordings(allRecordings.slice(startIndex, endIndex));
    } catch (error) {
          ...
          <div className="bg-white px-4 py-3 flex items-center justify-between border-t border-gray-200 sm:px-6">
            <div className="flex-1 flex justify-between sm:hidden">
              ...
              <button
                ...
                disabled={currentPage >= Math.ceil(totalRecordings / itemsPerPage)}
            ...
            <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
              <div>
                <p className="text-sm text-gray-700">
                  ...
                  <span className="font-medium">{Math.ceil(totalRecordings / itemsPerPage)}</span>
              ...
              <div>
                <nav className="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                  ...
                  {/* Page numbers */}
                  {[...Array(Math.min(5, Math.ceil(totalRecordings / itemsPerPage)))].map((_, index) => {
